...
int timekeeping_resume_set_rtctime(const struct timespec *tv)
{
	struct timespec ts_delta;
	unsigned long flags;
	...
	ts_delta.tv_nsec = tv->tv_nsec - xtime.tv_nsec;
	wall_to_monotonic = timespec_sub(wall_to_monotonic, ts_delta);
	total_sleep_time = timespec_add(total_sleep_time, ts_delta);
...
EXPORT_SYMBOL(timekeeping_resume_set_rtctime);
