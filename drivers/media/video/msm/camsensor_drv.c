...
typedef enum
{
  CAMSENSOR_KY_ADJUST_NONE = 0,
  CAMSENSOR_KY_ADJUST_INIT,
  CAMSENSOR_KY_ADJUST_FLASH,
  CAMSENSOR_KY_ADJUST_SCENE_AUTO,
  CAMSENSOR_KY_ADJUST_SCENE_PORTRAIT,
  CAMSENSOR_KY_ADJUST_SCENE_SCENERY,
  CAMSENSOR_KY_ADJUST_SCENE_NIGHT_PORTRAIT,
  CAMSENSOR_KY_ADJUST_SCENE_NIGHT_SCENERY,
  CAMSENSOR_KY_ADJUST_SCENE_ACTION,
}camsensor_ky_adjust_type;

typedef struct{
  unsigned long  partitionCode;
  unsigned long  dat_blk_No;
  unsigned long  regAdr;
  unsigned long  regDat;
}camsensor_Blk_struct;

typedef struct{
  camsensor_Blk_struct *p_bb_data;
  unsigned long data_cnt;
} camsensor_bb_data_struct;

static camsensor_bb_data_struct initial_data;

static camsensor_ky_adjust_type bb_adjust_type = CAMSENSOR_KY_ADJUST_NONE;

#define BB_MODEL_TYPE               0x000C5170
#define BB_AJUST_PATH               "/data"
#define BB_AJUST_FILENAME           "G-02_CAM_ADJ.bin"
#define BB_ADJTYPE_MAX_CODESIZE     8

#define BB_ADJTYPE_TYPE_INIT                0xFFFFFE01

#define BB_ADJTYPE_PARTITION_CODE_DATA_END  0xFFFFFFFE
#define BB_ADJTYPE_PARTITION_CODE_FILE_END  0xFFFFFFFF
...
#ifdef FEATURE_KYOCERA_MCAM_BB_ADJ
static int L_camsensor_drv_read_adjust_BB_value( void );
static void L_camsensor_drv_save_adjust_BB_value( unsigned long  marker, unsigned long data_cnt, char* p_BBdata );
static int L_camsensor_drv_write_adjust_BB_value( camsensor_bb_data_struct* p_data);
#endif
...
)
{
    ...
    if(rc == 0)
    {
        ...
        {
#ifdef FEATURE_KYOCERA_MCAM_BB_ADJ
            ERR_LOG("bb_adjust_type = [%x]\n", bb_adjust_type);

            if( bb_adjust_type & CAMSENSOR_KY_ADJUST_INIT )
            {
                printk("CAMSENSOR_KY_ADJUST_INIT TRUE\n");

                rc = L_camsensor_drv_write_adjust_BB_value( &initial_data );
                if( rc < 0 )
                {
                    ERR_LOG("BB ADJUST INIT I2C Write Err\n");
                }
            }
            else
            {
...
static int L_camsensor_drv_read_adjust_BB_value( void )
{
    ...
    unsigned long  model_type = 0;
    unsigned long  ver        = 0;
    unsigned long  marker     = 0;
    unsigned long  dat_cnt    = 0;
    unsigned char  temp[BB_ADJTYPE_MAX_CODESIZE];
    char buf[32];
    struct file *bb_adjust_file;
    char *BB_read_data = NULL;
    ...
    sprintf( buf, "%s/%s",BB_AJUST_PATH ,BB_AJUST_FILENAME );

    ERR_LOG("path  [%s] \n",buf);

    set_fs(KERNEL_DS);

    bb_adjust_file = filp_open( buf, O_RDONLY, 0);

    if( IS_ERR(bb_adjust_file) )
    {
        ERR_LOG("!!!!!!  FILE_OPEN_FAILED!\n");
    ...
    else
    {
        ...
        ret = TRUE;
        ERR_LOG("!!!!!!  FILE_OPEN_SUCCESSED! \n");
    }

    if(ret == TRUE)
    {
        read_cnt = bb_adjust_file->f_op->read( bb_adjust_file, temp, 4, &(bb_adjust_file->f_pos) );

        model_type =     (unsigned long )temp[0]
                      | ((unsigned long )temp[1] << 8)
                      | ((unsigned long )temp[2] << 16)
                      | ((unsigned long )temp[3] << 24)
                      ;

        ERR_LOG("DEBUG Model Type 0x%lx", model_type);

        if(model_type != BB_MODEL_TYPE)
        {
            ERR_LOG("DEBUG Model Type Not Match!!");

            filp_close(bb_adjust_file, NULL);
    ...
    if(ret == TRUE)
    {
        ...
        ver =    (unsigned long )temp[0]
              | ((unsigned long )temp[1] << 8)
              | ((unsigned long )temp[2] << 16)
              | ((unsigned long )temp[3] << 24)
              ;

        ERR_LOG("DEBUG Version 0x%lx", ver);
    ...
    while(ret)
    {
        ...
        marker =    (unsigned long )temp[0]
                 | ((unsigned long )temp[1] << 8)
                 | ((unsigned long )temp[2] << 16)
                 | ((unsigned long )temp[3] << 24)
                 ;

        ERR_LOG("BB_DEBUG Marker 0x%lx", marker);

        if(marker == BB_ADJTYPE_PARTITION_CODE_FILE_END)
        {
        ...
        else
        {
            ...
            dat_cnt =    (unsigned long )temp[0]
                      | ((unsigned long )temp[1] << 8)
                      | ((unsigned long )temp[2] << 16)
                      | ((unsigned long )temp[3] << 24)
                      ;

            ERR_LOG("BB_DEBUG data num [%ld]", dat_cnt);

            BB_read_data = (char *)vmalloc((dat_cnt * sizeof(camsensor_Blk_struct)) + 1);
            if(BB_read_data == NULL){
                ERR_LOG("malloc Fault. Down");
                filp_close(bb_adjust_file, NULL);
                ret = FALSE;
                break;
            }

            read_cnt = bb_adjust_file->f_op->read(bb_adjust_file, BB_read_data, (dat_cnt * sizeof(camsensor_Blk_struct)), &(bb_adjust_file->f_pos) );

            L_camsensor_drv_save_adjust_BB_value(marker, dat_cnt, BB_read_data);

            ERR_LOG("BB Read Data Free\n");
            ...
            ERR_LOG("BB_DEBUG Data End 0x%lx", marker);

            if(marker == BB_ADJTYPE_PARTITION_CODE_DATA_END)
            {

                read_cnt = bb_adjust_file->f_op->read( bb_adjust_file, temp, 8, &(bb_adjust_file->f_pos) );
            }
        }
    }
...
static void L_camsensor_drv_save_adjust_BB_value( unsigned long  marker, unsigned long data_cnt, char* p_BBdata )
{
    ...
    switch( marker )
    {
        case BB_ADJTYPE_TYPE_INIT:

            initial_data.p_bb_data = (camsensor_Blk_struct *)vmalloc(( data_cnt * sizeof(camsensor_Blk_struct)) + 1 );
            if( initial_data.p_bb_data == NULL )
            {

                ERR_LOG("Maker:0x%lx VmallocError\n", marker);
                return;
            }

            ERR_LOG("memcpy Start\n");
            memcpy( (void *)initial_data.p_bb_data, (void *)p_BBdata, (data_cnt * sizeof(camsensor_Blk_struct)));
            ERR_LOG("memcpy End\n");

            initial_data.data_cnt = data_cnt;

            bb_adjust_type |= CAMSENSOR_KY_ADJUST_INIT;
...
static int L_camsensor_drv_write_adjust_BB_value( camsensor_bb_data_struct* p_data)
{
    ...
    if(( p_data == NULL ) || ( p_data->p_bb_data == NULL ))
    {
        ERR_LOG("%s Parm NULL\n", __func__);
        return FALSE;
    }

    for( i=0; i < (p_data->data_cnt); i++ )
    {

        if( p_data->p_bb_data[i].regAdr == 0xFFFF )
        {
            mdelay( p_data->p_bb_data[i].regDat );
                    }
        else
        {
            rc = L_camsensor_drv_i2c_write( camsensor_drv_client->addr, p_data->p_bb_data[i].regAdr, p_data->p_bb_data[i].regDat, BYTE_LEN );
            if( rc < 0 )
            {
                ERR_LOG("I2C Write Err Addr:0x%04lx Data:0x%04lx\n", p_data->p_bb_data[i].regAdr, p_data->p_bb_data[i].regDat);
                rc = FALSE;
                break;
            }
    ...
    mdelay( 100 );
    ERR_LOG("mdelay 100ms\n");

    vfree(p_data->p_bb_data);
...
#ifdef FEATURE_KYOCERA_MCAM_BB_ADJ
{
    ...
    ret = L_camsensor_drv_read_adjust_BB_value();
    if( ret != TRUE )
    {
        ERR_LOG("L_camsensor_drv_read_adjust_BB_value Err\n");
    }
}
#endif
